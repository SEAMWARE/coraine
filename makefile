...
src/app/coraine/coraineStack.h: FORCE
	@{ echo '// GENERATED by the makefile from stackManifest.sh - do not edit, do not commit.'; 	   echo '#ifndef CORAINE_STACK_H_'; 	   echo '#define CORAINE_STACK_H_'; 	   echo ''; 	   echo 'static const char* const coraineStack[][2] ='; 	   echo '{'; 	   COR_HTTP_SERVER=$(COR_HTTP_SERVER) ./stackManifest.sh | while read -r lib sha; do printf '  { "%s", "%s" },\n' "$$lib" "$$sha"; done; 	   echo '  { NULL, NULL }'; 	   echo '};'; 	   echo ''; 	   echo '#endif  // CORAINE_STACK_H_'; 	 } > $@

#
# CMAKE_FEATURES - feature switches for a reduced build. E.g.
#
#   make di CMAKE_FEATURES=-DCOR_FEATURE_REGISTRATIONS=OFF BUILD_DEBUG=BUILD_DEBUG_MINIMAL
#
# CMake CACHES what it is given, so a switch stays in effect for that build
# DIRECTORY until it is passed again with the other value - which is why the
# example above also moves the build dir: it keeps the ordinary BUILD_DEBUG a
# full build instead of quietly turning it into a reduced one that then installs
# over /usr/local/bin/coraine.
#
CMAKE_FEATURES ?=

#
# src/app/coraine/coraineBuild.h - what produced this binary.
#
# GENERATED, never committed - the sibling of coraineStack.h above, and split
# from it because they answer different questions: the stack says WHAT IS LINKED
# IN, this says WHERE THIS BINARY CAME FROM. Both are regenerated on every build
# rather than cached, for the same reason: a stale answer here is worse than no
# answer, and it is two shell commands.
#
# The build TYPE and the compiler are not here - only cmake knows those, and it
# passes them as -D (see CMakeLists.txt). Each fact comes from whoever actually
# knows it.
#
src/app/coraine/coraineBuild.h: FORCE
	@{ echo '// GENERATED by the makefile - do not edit, do not commit.'; \
	   echo '#ifndef CORAINE_BUILD_H_'; \
	   echo '#define CORAINE_BUILD_H_'; \
	   echo ''; \
	   echo '#define CORAINE_GIT_SHA  "$(CORAINE_GIT_SHA)"'; \
	   echo '#define CORAINE_BUILT_AT "$(CORAINE_BUILD_AT)"'; \
	   echo ''; \
...
coverage: src/app/coraine/coraineStack.h src/app/coraine/coraineBuild.h
#
