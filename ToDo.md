...
This file is the working backlog. The prior audit's already-closed items and
phantoms are not duplicated here.

Methodology: 5 parallel agents, one per spec slice (§ 5.6/5.7, § 5.8, § 5.9-5.14,
temporal+§ 5.16+§ 5.15, ch4/ch6/ch7), grepping `coraine/src/` and
`corNgsild/` against the official spec markdown (`ts-104-175/md/clause-*.md`).
...
| # | § | Gap | Status |
|---|---|-----|--------|
| 1 | 5.7.2 / 6.4.3.2 | `?containedBy=` URL param silently ignored (join cycle prevention, 0..1 optional) | ✅ done 2026-05-01; functest `url_param_contained_by` |
| 2 | 5.7.2 / 6.23.3.1 | `?expandValues=` URL param not implemented (JSON-LD type-coercion expansion, MAY-not-MUST) | ✅ already implemented (`ldQParse.c:531-548`); the audit was wrong |
| 3 | 5.2.12 | `ngsildConformance` member on Subscription accepted but not applied — TODO already at `ldDistOp.c:220` (back-compat path for older brokers) | ✅ done 2026-05-02 (corNgsild). New `ldConformanceDowngrade` module implements § 4.3.6.8 Tables 4.3.6.8-1/2/3 transformations: < 1.9 strips entity+attr expiresAt and attr valueType; < 1.8 reformats Json/Vocab/List/ListRelationship to Property/Relationship; < 1.4 strips scope and reformats LanguageProperty; < 1.3 strips datasetId/observedAt/unitCode and collapses multi-instance arrays + multi-type arrays. Validated as `M.m`; applied to subscription notification body before render. Distop forward back-compat (`ldDistOp.c:220` TODO) still pending — separate scope. Functests `subscription_ngsild_conformance` / `subscription-ngsild-conformance` |
| 4 | 5.10.2 | Discovery `GET /csourceRegistrations` returns full RegistrationInfo even when only a subset matches the filter — § 5.10.2.5 wants the response filtered | ✅ done 2026-05-01; functest `csource_reg_discovery_filtered_info` |
| 5 | 5.10.2 / 6.3.10 | Discovery returns 200 even when results > limit — should emit Link rel=next/prev/first (audit's "206 + Content-Range" was overstated; per § 6.3.10 that's temporal-only). Also `NGSILD-Results-Count` on `?count=true`. | ✅ done 2026-05-01; functest `csource_reg_discovery_pagination` |
| 6 | 5.11.7 / 5.11.2 | `timeInterval` periodic CSR-subscription notifications | ✅ done 2026-05-01; refactored `ldPernotLoop` into a shared `ldPeriodicLoop` engine (1-Hz tick over registered consumers); pernot + CSR-Sub timeInterval both register with it. Functest `csr_subscription_time_interval` |
| 7 | 6.3.2 | `405 Method Not Allowed` returned without the required `Allow:` header | ✅ done 2026-05-01; functest `http_405_method_not_allowed` |
| 8 | 6.3.2 | `411 Length Required` and `413 Request Entity Too Large` — no handlers (transport-level limits not enforced) | ✅ done 2026-05-01; broker `--maxRequestSize` / `-mrs` MiB CLI (default 2 MiB, 0 = no cap; robotics use-case for large attribute payloads). 411 emitted body-less per § 6.3.4; 413 streaming + Content-Length pre-check. Functest `http_411_413_body_limits` |
...
| # | Gap | Status |
|---|-----|--------|
| 9 | `notifierInfo.MQTT-Version` ignored — hardcoded; spec default mqtt5.0 | ✅ done 2026-05-02 (corNgsild). Coraine honors mqtt5.0 / mqtt3.1.1 via libmosquitto's `MOSQ_OPT_PROTOCOL_VERSION`, and validates notifierInfo (rejects bad QoS / Version values). Functest `subscription_notify_mqtt_validation` |
...
| 11 | MQTT QoS=1 / QoS=2 + MQTT-Version round-trip functest | ✅ done 2026-05-02. Functest `subscription_notify_mqtt_qos_version` exercises QoS=1+mqtt3.1.1 and QoS=2+mqtt5.0 |
...
| # | § | Gap | Status |
|---|---|-----|--------|
| 12 | 5.14.4.4 | `linkedMaps` in distributed EntityMap creation — was emitted empty | ✅ done 2026-05-02 (corNgsild + getEntities). Forward branch: when `entityMapCreate=true`, switches from `GET /entities` to `GET /entityMaps` (the spec route for Create-EntityMap-Query-Entity); CP returns its own EntityMap with `id` + `entityMap` keys; local broker stores `(csr.regId → remote-EntityMap-id)` in `linkedMaps`. Functests `entitymap_linked_maps` / `entitymap-linked-maps`. Pagination optimization (using stored remote map id during page-fetch) deferred — current per-entity GET still works. |
...
| 15 | 6.3.18 | `local=true` parameter coverage on `/types/`, `/attributes/`, `/temporal/entities/` | ✅ done 2026-05-02 (corNgsild). `LD_PARAM_LOCAL` added to `LD_PARAMS_GET_TEMPORAL_ENTITY` and `LD_PARAMS_DELETE_TEMPORAL_ATTR` (was 400 unknown-param). Functest `local_param_distops_suppress`. Note: csourceSubscriptions excluded — Registry API is always local per § 6.3.18 |
...
Process Tier A in numbered order (1 → 8). Each item: implement, functest,
commit + push. After Tier A, reassess priorities for B / C / D against
real-world demand.
