...
`corNgsild/` against the spec markdown under `~/git/fwLibs/claude/spec/`.
...
| # | § | Gap | Status |
|---|---|-----|--------|
| 12 | 5.14.4.4 | `linkedMaps` in distributed EntityMap creation — was emitted empty | ✅ done 2026-05-02 (corNgsild + fwNgsild + getEntities both stacks). Forward branch: when `entityMapCreate=true`, switches from `GET /entities` to `GET /entityMaps` (the spec route for Create-EntityMap-Query-Entity); CP returns its own EntityMap with `id` + `entityMap` keys; local broker stores `(csr.regId → remote-EntityMap-id)` in `linkedMaps`. Functests `entitymap_linked_maps` / `entitymap-linked-maps`. Pagination optimization (using stored remote map id during page-fetch) deferred — current per-entity GET still works. |
