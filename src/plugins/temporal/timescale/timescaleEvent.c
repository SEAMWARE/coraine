...
int timescaleExecEntityInsertLocked(const TroeEvent* evP)
{
  ...
  const char* tenantName = (evP->tenantP != NULL) ? evP->tenantP->name : "";

  const char* paramV[4];
  paramV[0] = tenantName;
  paramV[1] = evP->entityId   != NULL ? evP->entityId   : "";
  paramV[2] = evP->entityType != NULL ? evP->entityType : "";
  paramV[3] = opName(evP->op);

  char sql[512];
  snprintf(sql, sizeof(sql),
           "INSERT INTO troe_entities (tenant, entity_id, entity_type, modified_at, op) "
           "VALUES ($1, $2, $3, %s, $4) "
           "ON CONFLICT (tenant, entity_id, modified_at, op) DO NOTHING",
           tsExpr);

  PGresult* res = PQexecParams(timescaleConn, sql, 4, NULL, paramV, NULL, NULL, 0);
...
int timescaleExecAttrInsertLocked(const TroeEvent* evP)
{
  ...
  // Bind params:
  //   $1 tenant, $2 entity_id, $3 entity_type, $4 attr_name, $5 dataset_id,
  //   $6 op, $7 attr_kind, $8 observed_at iso, $9 v_text, $10 v_number,
  //   $11 v_bool, $12 v_datetime, $13 v_compound, $14 sub_attrs
  const char* paramV[14];
  char        kindBuf[16];
  ...
  paramV[1]  = evP->entityId   != NULL ? evP->entityId   : "";
  paramV[2]  = evP->entityType != NULL ? evP->entityType : "";
  paramV[3]  = evP->attrName   != NULL ? evP->attrName   : "";
  paramV[4]  = (evP->datasetId != NULL && evP->datasetId[0] != 0) ? evP->datasetId : cols.dsId;
  paramV[5]  = opName(evP->op);
  paramV[6]  = kindBuf;
  paramV[7]  = cols.observedAtIso;
  paramV[8]  = cols.v_text;
  paramV[9]  = cols.v_number;
  paramV[10] = cols.v_bool;
  paramV[11] = cols.v_datetime;
  paramV[12] = cols.v_compound;
  paramV[13] = cols.sub_attrs;
  ...
  snprintf(sql, sizeof(sql),
           ...
           "  tenant, entity_id, entity_type, attr_name, dataset_id, created_at, modified_at, op, attr_kind, "
           "  observed_at, v_text, v_number, v_bool, v_datetime, v_compound, sub_attrs) "
           "VALUES ($1, $2, $3, $4, $5, %s, %s, $6, $7::smallint, "
           "        $8::timestamptz, $9, $10::double precision, $11::boolean, $12::timestamptz, "
           "        $13::jsonb, $14::jsonb)",
           tsExpr, tsExpr);

  PGresult* res = PQexecParams(timescaleConn, sql, 14, NULL, paramV, NULL, NULL, 0);
  ExecStatusType st = PQresultStatus(res);
  if (st != PGRES_COMMAND_OK)
  {
