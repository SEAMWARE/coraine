//
// FILE            mongocRegistrationDelete.c
//
// AUTHOR          Ken Zangelin
//
...
#include "currentState/mongoc/mongocRegistrationDelete.h"  // Own interface
...
//
int mongocRegistrationDelete(Tenant* tenantP, const char* regId)
{
  ...
  if (!ok)
  {
    KT_E("mongoc: registrationDelete failed: %s", error.message);
    result = DB_ERR;
  }
  else
  {
