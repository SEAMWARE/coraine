...
#include "currentState/swRamDB/ramdbGeoMatch.h"          // Own interface
...
// -----------------------------------------------------------------------------
//
// ramdbGeoInit / ramdbGeoClose
//
void ramdbGeoInit(void)
{
  geosCtx = GEOS_init_r();
}

void ramdbGeoClose(void)
{
  if (geosCtx != NULL)
  {
...
//
bool ramdbGeoMatch(KjNode* entityP, DbQueryFilter* filterP, double* distanceP)
{
  if (distanceP != NULL)
    *distanceP = -1;
