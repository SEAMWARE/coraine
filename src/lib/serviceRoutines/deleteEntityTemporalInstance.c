...
bool deleteEntityTemporalInstance(void)
{
  ...
  {
    troeNotAvailable("temporal-instance delete");
    return true;
  }
    ...
    if (!ldDistOpLoopDetected(ownAlias))
    {
      ...
      for (int m = 0; m < 3; m++)
      {
        ...
        for (int i = 0; i < matchN; i++)
        {
          ...
          if (!ldRegOpSupported(csr, LdOpDeleteAttrInstanceTemporal)) continue;
          if (!csrCoversAttr(csr, attrIri))                           continue;

          const char* upErr  = NULL;
          int         upCode = forwardDeleteInstance(csr, entityId, attrWild, instanceId, ownAlias, &upErr);
  ...
  else if (!localOk && !localNotFound)
  {
    ...
    snprintf(detail, sizeof(detail),
             "local instance delete failed for '%s'/'%s'/'%s'",
